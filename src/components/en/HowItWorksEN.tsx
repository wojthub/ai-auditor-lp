...
const steps = [
  {
    ...
    description: 'Enter the page address and keyword. The tool automatically fetches content and SERP data - no manual copy-paste.',
    icon: (
    ...
    <section id="how-it-works" style={{
      ...
      <div style={{ maxWidth: 1024, margin: '0 auto', paddingLeft: 24, paddingRight: 24, position: 'relative', zIndex: 2 }}>
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.6 }} style={{ textAlign: 'center', marginBottom: 56 }}>
          <h2 style={{ fontSize: 'clamp(1.75rem, 4vw, 2.25rem)', fontWeight: 600, color: '#ffffff', letterSpacing: '-0.025em' }}>
            How it works
          </h2>
        </motion.div>
          ...
          {steps.map((step, i) => (
            <Fragment key={step.number}>
              <motion.div
                ...
                viewport={{ once: true, margin: '-40px' }} transition={{ duration: 0.5, delay: i * 0.13 }}
                style={{ background: '#ffffff', border: '1px solid #dfe1e7', borderRadius: 8, padding: '32px 28px 28px', display: 'flex', flexDirection: 'column', height: '100%' }}
              >
                <motion.div whileHover={{ scale: 1.08, rotate: 4, transition: { type: 'spring', stiffness: 400, damping: 20 } }}
                  style={{ width: 88, height: 88, borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ffffff', background: '#0b7983', border: '1px solid rgba(255,255,255,0.2)', marginBottom: 24, marginTop: 4 }}>
                  {step.icon}
                </motion.div>
                <h3 style={{ fontSize: 17, fontWeight: 600, color: '#0d0d12', marginBottom: 10, letterSpacing: '-0.02em', lineHeight: 1.3 }}>{step.title}</h3>
                <p style={{ fontSize: 14, color: '#36394a', lineHeight: 1.65 }}>{step.description}</p>
              </motion.div>
              {i < steps.length - 1 && (
                <motion.div key={`connector-${i}`} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.13 + 0.2 }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <StepConnector />
                </motion.div>
              )}
            </Fragment>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-40px' }} transition={{ duration: 0.5, delay: 0.3 }} style={{ textAlign: 'center', marginTop: 48 }}>
          <motion.a href={`${APP_URL}/login?lang=en`} whileHover={{ scale: 1.03, transition: { type: 'spring', stiffness: 400, damping: 20 } }} whileTap={{ scale: 0.97 }}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '14px 28px', borderRadius: 8, background: '#ffffff', color: '#0b7983', border: '1px solid rgba(255,255,255,0.5)', fontWeight: 600, fontSize: 15, textDecoration: 'none', letterSpacing: '-0.01em' }}>
            Run audit
      ...
      <style>{`
        .howitworks-grid { display: grid; grid-template-columns: 1fr auto 1fr auto 1fr; gap: 0 16px; align-items: stretch; }
        .step-connector { display: flex; align-items: center; justify-content: center; }
        @media (max-width: 768px) { .howitworks-grid { grid-template-columns: 1fr; gap: 12px; } .step-connector { display: none !important; } }
      `}</style>
    </section>
  );
}
