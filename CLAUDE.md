...
- **Pastylka potrzebuje `position: relative` + `z-index: 1`.** Urządzenie ma `transform` i `filter`, więc tworzy kontekst układania i maluje się NAD zwykłymi blokami w przepływie - bez tego adnotacja przeniesiona pod mockup po prostu znika za nim, mimo że jest dalej w DOM. Ląduje w strefie, gdzie maska wygasza ekran, i to jest zamierzone.
