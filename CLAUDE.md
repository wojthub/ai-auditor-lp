...
**Showcase** ([Showcase.tsx](src/components/Showcase.tsx) / [en/ShowcaseEN.tsx](src/components/en/ShowcaseEN.tsx)) - mockup panelu w ramce urządzenia (cienki bezel, metaliczna krawędź, połysk), zanikający ku dołowi przez `maskImage`. Bez copy/CTA. Umieszczony zaraz po Hero - górny kawałek mockupu widoczny above the fold (Showcase `padding: 26px 0 96px` — górne 76px zeszło do 26px, gdy nad urządzeniem stanęła adnotacja i dziura pod Hero zrobiła się za duża; Hero zredukowany: bez `minHeight`/flex-center, `padding: 40px 24px 28px`). Mockup `animate` z `delay: 1` - wjeżdża od dołu po 1 s.
