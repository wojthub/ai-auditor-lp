...
Na ekranie leżą **trzy slajdy** (od 2026-09-04) w kolejności pracy z audytem: Podsumowanie → Rekomendacje → Wdrożenie w treść. Karuzelę trzyma wspólny [ShowcaseDeck.tsx](src/components/ShowcaseDeck.tsx) - `Showcase`/`ShowcaseEN` podają mu tylko slajdy i etykiety (ten sam wzorzec co `DimensionPage` z propem `t`). Slajdy **przełączają się same co 5,2 s** i wracają z ostatniego na pierwszy; pauzę robi kursor nad karuzelą i trwające przeciągnięcie, a `prefers-reduced-motion: reduce` wyłącza cykl całkiem. Ręczna zmiana slajdu **nie kończy** autoodtwarzania - odlicza od nowa. Jedyna klikalna kontrolka to **strzałki** na ekranie (pigułek z nazwami zakładek świadomie nie ma, decyzja z 2026-09-04); poza nimi działa swipe/drag. Strzałka na krańcu talii gaśnie (`opacity: 0` + `pointer-events: none`), więc na pierwszym slajdzie widać tylko „dalej".

- **Nad urządzeniem stoi adnotacja** - kropka + nazwa widocznej zakładki (crossfade przez `AnimatePresence`) + trzy kreski pozycji + licznik `1/3`. Kreski **przeskakują wprost na wybrany slajd** (od 2026-09-07): każda to `button` z `aria-label` równym nazwie zakładki, a pole trafienia (19 px) robi `padding` przycisku, nie sam 3-pikselowy pasek - dzięki temu wysokość pastylki się nie zmienia. Pudełko etykiety ma stałą wysokość, inaczej crossfade podskakuje.
